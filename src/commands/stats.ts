				...
				fields: [
					{
						...
						value: [
							...
							`Server was created at: ${moment(config.guild.createdAt).format(
								"YYYY/M/D"
							)}`,
							`Num. of channels: ${
								config.guild.channels.cache.filter(c => c.type != "category")
									.size
							}`,
					...
					},
					{
						...
						value: [
							`Uptime: ${duration(client.uptime, "milliseconds").humanize()}`,
						].join("\n"),
						inline: false,
					},
				],
...
console.log(duration("45342562", "milliseconds").humanize());
